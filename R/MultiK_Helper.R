...
MultiK <- function(seu, resolution = seq(0.05, 2, 0.05), nPC = 30, reps = 100, pSample = 0.8, seed = NULL, vars.to.regress = NULL, batch = "orig.ident", integrated.assay.norm.method = "LogNorm") {
  ...
  for(i in 1: reps) {
    ...
    if (DefaultAssay(subX) == "integrated") {
      ...
      # Scaling unwanted variation for LogNorm-normalised object
      if (integrated.assay.norm.method == "LogNorm") {
        all.genes <- rownames(x = subX)
        subX <- ScaleData(object = subX, features = all.genes, vars.to.regress = vars.to.regress, verbose = F)
      }

      # TODO: re-scale subset of SCT-normalised integrated object
...
findResol <- function(seu, ks, optK) {
  ...
  if (DefaultAssay(seu) == "integrated") {
    L <- as.numeric(gsub("integrated_snn_res.", "", names(which(ks < optK))))
    R <- as.numeric(gsub("integrated_snn_res.", "", names(which(ks > optK)[1])))
    }
